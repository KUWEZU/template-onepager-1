    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-12">
              ...
              {[{ label: "Facebook", href: "#" }, { label: "Instagram", href: "#" }].map(({ label, href }) => (
                <a key={label} href={href} aria-label={`${label} öffnen`}
                  className="w-10 h-10 rounded-lg bg-white/[0.06] border border-white/[0.08] flex items-center justify-center
                             text-white/40 hover:text-white hover:border-white/[0.16] transition-all text-xs font-medium min-h-[44px]">
                  {label.slice(0, 2)}
                </a>
              ))}
            </div>
          ...
          {/* Leistungen */}
          <div>
            <h3 className="text-base font-semibold text-white mb-5 uppercase tracking-wide text-sm">Leistungen</h3>
            <ul className="space-y-2.5" role="list">
              {client.leistungen.slice(0, 8).map((l) => (
                <li key={l.slug}>
                  <a href="#leistungen"
                    className="text-base text-white/50 hover:text-white transition-colors leading-relaxed">
                    {l.title}
                  </a>
                </li>
              ))}
          ...
          {/* Unternehmen */}
          <div>
            <h3 className="text-base font-semibold text-white mb-5 uppercase tracking-wide text-sm">Unternehmen</h3>
            <ul className="space-y-2.5" role="list">
              {[
                ...
                <li key={label}>
                  <a href={href} className="text-base text-white/50 hover:text-white transition-colors">{label}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Kontaktdaten */}
          <div>
            <h3 className="text-base font-semibold text-white mb-5 uppercase tracking-wide text-sm">Kontakt</h3>
            <ul className="space-y-4" role="list">
              {client.telefon && (
                <li>
                  <a href={`tel:${client.telefon}`}
                    className="flex items-center gap-3 text-base text-white/50 hover:text-white transition-colors group min-h-[44px]">
              ...
              {client.adresse && (
                <li>
                  <address className="not-italic flex items-start gap-3 text-base text-white/50">
      ...
      <div className="border-t border-white/[0.05]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-white/30">© {year} {client.name}. Alle Rechte vorbehalten.</p>
          <div className="flex items-center gap-5">
            <a href="/impressum"        className="text-sm text-white/30 hover:text-white/70 transition-colors">Impressum</a>
            <a href="/datenschutz"      className="text-sm text-white/30 hover:text-white/70 transition-colors">Datenschutz</a>
            <a href="/barrierefreiheit" className="text-sm text-white/30 hover:text-white/70 transition-colors">Barrierefreiheit</a>
          </div>
        </div>
      </div>
    </footer>
