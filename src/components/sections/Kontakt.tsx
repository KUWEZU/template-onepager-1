    ...
    <section id="kontakt" className="py-24 bg-brand-secondary" aria-labelledby="kontakt-heading">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-14">
          ...
          <p className="max-w-xl mx-auto text-brand-text/55 text-lg">Vereinbaren Sie einen Termin, stellen Sie uns eine Frage oder besuchen Sie uns direkt.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          <div>
            <div className="grid sm:grid-cols-2 gap-4 mb-8">
              {INFOS.map(({ icon: Icon, title, lines, href }) => (
                <div key={title} className="bg-brand-bg border border-brand-text/[0.08] rounded-xl p-5">
                  ...
                  {lines.map((line, i) =>
                    href && i === 0 ? (
                      ...
                      <p key={i} className="text-sm text-brand-text/50">{line}</p>
                    )
                  )}
                </div>
              ))}
            </div>
            <div className="h-52 bg-brand-bg border border-brand-text/[0.08] rounded-xl flex items-center justify-center" aria-label="Kartenansicht" role="img">
          ...
          <div className="bg-brand-bg border border-brand-text/[0.08] rounded-2xl p-6 sm:p-8">
            {submitted ? (
              <div className="h-full flex flex-col items-center justify-center text-center py-12">
                ...
                <p className="text-brand-text/55 max-w-xs">Vielen Dank für Ihre Anfrage. Wir melden uns schnellstmöglich.</p>
              </div>
            ) : (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input id="name" name="name" type="text" autoComplete="name" required value={form.name} onChange={handleChange} placeholder="Max Mustermann"
                      className="w-full px-4 py-3 bg-brand-surface border border-brand-text/[0.12] hover:border-brand-text/[0.20] focus:border-brand-primary/50 rounded-xl text-brand-text placeholder:text-brand-text/30 text-sm outline-none transition-colors" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea id="message" name="message" rows={4} required value={form.message} onChange={handleChange} placeholder="Wie können wir Ihnen helfen?"
                      className="w-full px-4 py-3 bg-brand-surface border border-brand-text/[0.12] hover:border-brand-text/[0.20] focus:border-brand-primary/50 rounded-xl text-brand-text placeholder:text-brand-text/30 text-sm outline-none transition-colors resize-none" />
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <label htmlFor="dsgvo" className="text-xs text-brand-text/50 leading-relaxed cursor-pointer">
                  ...
                  <button type="submit" disabled={loading}
                    className="w-full flex items-center justify-center gap-2 py-3.5 bg-brand-primary text-brand-bg font-semibold rounded-xl hover:bg-brand-primary-hover transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {loading ? (
                      <><span className="w-4 h-4 border-2 border-brand-bg/30 border-t-brand-bg rounded-full animate-spin" />Sende...</>
                    ) : (
